...
#define REC_BAUD 230400      //Define the reccomended baud rate
...
#define SUBMIT_CALLBACK LoRa.onReceive(Receive);//Submit callback funtion when class object is constructed
...
public:    
  ...
  //This function only specifies the variables. InitRadio() must be called after hw pins are configured
...
private:
  ...
  int _implicit_header = 0;      // Set header mode
  ...
  long _sig_bandwidth = 125000;  // Signal Bandwidth
  int _coding_rate = 8;          // Coding Rate denominator
  long _lora_preamble = 8;       // Set lora radio preamble length
